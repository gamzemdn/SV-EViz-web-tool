...
def generate_evalsvcallers_visuals(pivot_ref, df_long, df_block):
    ...
    metric_order = ["Call", "Recall", "Precis", "F1-Score"]
    df_metrics_all = df_block.copy()
    df_metrics_all["Metric_Order"] = df_metrics_all["Metric"].apply(
        lambda m: metric_order.index(m) if m in metric_order else 999
    )
    df_metrics_all = df_metrics_all.sort_values(by=["SVTYPE", "SizeClass", "Metric_Order"]).drop(columns=["Metric_Order"])

    # Stacked bar for reference summary
    bar_fig = px.bar(
    ...
    line_fig = px.line(
        df_long, x="Read Count", y="Value",
        color="Metric", line_dash="SizeClass", facet_row="SVTYPE",
        markers=True, title="Read Support Metrics by SV Type, Metric and Size Class"
    )
    line_fig.update_layout(height=1200)
    ...
    return html.Div([
        ...
        dash_table.DataTable(
            data=df_metrics_all.to_dict("records"),
            columns=[{"name": col, "id": col} for col in df_metrics_all.columns],
            style_table={'overflowX': 'auto'},
            page_size=20,
            style_cell={'textAlign': 'center'}
        ),
